!
! Rotated-axis gradients against the Rys gradient path.
!
! int2e_ip1_rotaxis_cart/sph compute what int2e_ip1_cart/sph do -- d/dA of
! the two-electron integral, in libcint's sign and layout -- by the
! rotated-axis expansion instead of Rys quadrature.  The reference is
...
program rotaxis_grad_check
   ...
   use cint_rotaxis_grad_2e, only: int2e_ip1_rotaxis_cart, int2e_ip1_rotaxis_sph, rotaxis_grad_supported
   ...
   if (nbad > 0) stop 1
   print '(A)', "  RESULT: PASS (rotated-axis gradients reproduce Rys)"
...
contains
   ...
   subroutine sweep(stride, label)
      ...
      do l = 0, nbas-1, stride
         shls = [i, j, k, l]
         if (.not. rotaxis_grad_supported(shls, bas)) then
            nskip = nskip + 1
            cycle
         end if
         if (sph == 1) then
         ...
         if (sph == 1) then
            ...
            hv2 = int2e_ip1_rotaxis_sph(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         else
            hv1 = int2e_ip1_cart(rys, dims, shls, atm, natm, bas, nbas, env, ws)
            hv2 = int2e_ip1_rotaxis_cart(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         end if
         nq = nq + 1
...
end program rotaxis_grad_check
