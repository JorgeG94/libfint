!
! A gradient contraction: the two-electron gradient paths compared where it
! matters, and the only check that runs them under OpenMP.
!
...
! evaluates the quartet.  The quantity assembled is the two-electron part
! of the nuclear gradient, contracted against a density -- the real
! workload rather than a bare integral loop.  It is a test as well: the
! paths must agree on the assembled gradient, which also says the drivers
! are re-entrant and survive a threadprivate workspace.
!
...
program grad_bench
   ...
   allocate(dm(nao,nao), gref(3,MAXA))
   call fake_density()
   dmax = maxval(abs(dm))
   call schwarz()
   ...
   do ipath = 0, 3
      ...
      if (ipath == 0) then
         gref = gr(:,:,0)
         scal = max(maxval(abs(gref)), 1.0e-30_dp)
         print '(A,A,F9.3,A)', "  ", pname(ipath), tsec(ipath), " s"
      else
         rel = maxval(abs(gr(:,:,0) - gref))/scal
         worst = max(worst, rel)
   ...
   if (only == 0) then
      ...
      print '(A,ES10.2,A)', "  RESULT: PASS (both paths agree to ", worst, " scaled)"
   end if
...
contains
   ...
   subroutine fock(path)
      ...
      integer  :: a, b, c, d, ii, jj, kk, ll, shls(0:3), dims(0:3), tid, at, ax, nq4
      real(dp) :: v, s4
      real(dp), allocatable :: buf(:)
      logical  :: hv

      allocate(gr(3, MAXA, 0:nthr-1))
      gr = 0.0_dp
      !$omp parallel default(shared) private(ip_,kp_,ish,jsh,ksh,lsh,di,dj,dk,dl, &
      !$omp   a,b,c,d,ii,jj,kk,ll,shls,dims,v,s4,buf,hv,tid,at,ax,nq4)
      tid = 0
      !$ tid = omp_get_thread_num()
      allocate(buf(0:3*15**4-1))
      ...
      do ip_ = 1, npair
         do kp_ = 1, ip_
            ...
            ! d/dA rides on the atom the FIRST shell sits on
            at = bas(BAS_SLOTS*ish + ATOM_OF) + 1
            do ax = 0, 2
            do d = 0, dl-1
            do c = 0, dk-1
            do b = 0, dj-1
            do a = 0, di-1
               v = buf(a + di*(b + dj*(c + dk*d)) + ax*nq4)*s4
               if (abs(v) < 1.0e-14_dp) cycle
               ii = ao_loc(ish)+a; jj = ao_loc(jsh)+b
               kk = ao_loc(ksh)+c; ll = ao_loc(lsh)+d
               gr(ax+1, at, tid) = gr(ax+1, at, tid) &
                  + v*(dm(ii+1,jj+1)*dm(kk+1,ll+1) - 0.25_dp*dm(ii+1,kk+1)*dm(jj+1,ll+1))
            end do
            end do
            end do
            end do
