!
! Rotated-axis McMurchie-Davidson against the Rys path.
!
! int2e_rotaxis_cart/sph compute the same integrals int2e_cart/sph do, by a
! different algorithm: the Hermite expansion in a frame rotated so that the
! bra pair lies on z and the ket pair in the xz plane, the Boys function in
! place of the Rys roots.  THIS TEST CANNOT BE BIT-IDENTICAL, and not
...
program rotaxis_check
   ...
   if (nbad > 0) stop 1
   print '(A)', "  RESULT: PASS (rotated-axis MMD reproduces Rys)"
...
contains
   ...
   ! The public wrappers in libfint.f90: libcint_2e_rotaxis_cart/sph against
   ! libcint_2e_cart/sph through the same (buf, shls, atm, natm, bas, nbas,
   ! env) list mqc uses, plus libcint_rotaxis_supported, on 6-31G.
   subroutine abi()
      use libcint_fortran, only: libcint_2e_cart, libcint_2e_sph, libcint_2e_rotaxis_cart, &
                         libcint_2e_rotaxis_sph, libcint_rotaxis_supported
      use iso_c_binding, only: ip => c_int
      integer(4), allocatable :: catm(:,:), cbas(:,:)
      real(dp),   allocatable :: cenv(:)
      integer(4) :: coff, cnbas
      integer(ip) :: shls(4), ret, nb, i, j, k, l, sph, n
      real(dp), allocatable :: a(:), b(:)
      integer :: stride, nsup, shl(0:3)

      allocate(catm(ATM_SLOTS, NATM), cbas(BAS_SLOTS, MAXBAS), cenv(20000))
      catm = 0; cbas = 0; cenv = 0.0_dp
      coff = PTR_ENV_START
      call setup_c2h6_geometry(catm, cenv, coff)
      call setup_reference_basis(1, cbas, cenv, cnbas, coff)
      nb = int(cnbas, ip)
      allocate(a(6**4), b(6**4))
      stride = max(1, int(nb)/4)
      nsup = 0
      do sph = 0, 1
      do i = 0, nb-1, stride
      do j = 0, nb-1, stride
      ...
      do l = 0, nb-1, stride
         shls = [i, j, k, l]
         if (.not. libcint_rotaxis_supported(shls, int(cbas, ip), nb)) cycle
         nsup = nsup + 1
         n = int(cint_cgto_cart(int(i), reshape(int(cbas(:,1:nb)), [BAS_SLOTS*int(nb)])), ip)
         a = 0; b = 0
         if (sph == 1) then
            ret = libcint_2e_sph(a, shls, int(catm, ip), int(NATM, ip), int(cbas, ip), nb, cenv)
            ret = libcint_2e_rotaxis_sph(b, shls, int(catm, ip), int(NATM, ip), int(cbas, ip), nb, cenv)
         else
            ret = libcint_2e_cart(a, shls, int(catm, ip), int(NATM, ip), int(cbas, ip), nb, cenv)
            ret = libcint_2e_rotaxis_cart(b, shls, int(catm, ip), int(NATM, ip), int(cbas, ip), nb, cenv)
         end if
         shl = int(shls)
         call compare(a, b, size(a), "abi", int(sph), shl)
      end do
      end do
      end do
      end do
      ...
      if (nsup == 0) then
         print '(A)', "  FAIL: libcint_rotaxis_supported accepted nothing in 6-31G"
         nbad = nbad + 1
      end if
      nq = nq + nsup
   end subroutine abi
   ...
   subroutine sweep(stride, label)
      ...
      do l = 0, nbas-1, stride
         shls = [i, j, k, l]
         if (.not. rotaxis_supported(shls, bas)) then
            nskip = nskip + 1
            cycle
         end if
         if (sph == 1) then
         ...
         if (sph == 1) then
            ...
            hv2 = int2e_rotaxis_sph(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         else
            hv1 = int2e_cart(rys, dims, shls, atm, natm, bas, nbas, env, ws)
            hv2 = int2e_rotaxis_cart(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         end if
         nq = nq + 1
