!
! Obara-Saika/HGP gradients against the Rys gradient path.
!
! int2e_ip1_hgp_cart/sph compute what int2e_ip1_cart/sph do -- d/dA of the
! two-electron integral, in libcint's sign and layout -- by Obara-Saika
! with the Head-Gordon-Pople split instead of Rys quadrature.  The factor
! of 2a that a derivative brings varies per primitive while the transfers
! run once per contracted quartet, so it rides in a second contracted set;
! doc/HGP.md has it.  The reference is
! libfint's own int2e_ip1, which catalogue_check holds bit-identical to
! libcint, so this is transitively a comparison with the C.  THIS TEST CANNOT BE BIT-IDENTICAL, and not
...
program hgp_grad_check
   ...
   if (nbad > 0) stop 1
   print '(A)', "  RESULT: PASS (HGP gradients reproduce Rys)"
...
contains
   ...
   subroutine sweep(stride, label)
      ...
      do l = 0, nbas-1, stride
         shls = [i, j, k, l]
         if (.not. hgp_grad_supported(shls, bas)) then
            nskip = nskip + 1
            cycle
         end if
         if (sph == 1) then
         ...
         if (sph == 1) then
            hv1 = int2e_ip1_sph(rys, dims, shls, atm, natm, bas, nbas, env, ws)
            hv2 = int2e_ip1_hgp_sph(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         else
            hv1 = int2e_ip1_cart(rys, dims, shls, atm, natm, bas, nbas, env, ws)
            hv2 = int2e_ip1_hgp_cart(rot, dims, shls, atm, natm, bas, nbas, env, ws)
         end if
         nq = nq + 1
