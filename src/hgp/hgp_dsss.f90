...
contains
   ! (ds|ss)  dsss: vrr=15 carried=6 hrr=6 in 1 block(s) comps=6
   subroutine hgp_dsss(nbra, ncb, bp, kab, nket, nck, kp, kcd, ab, cd, cutoff, res, any)
      ...
      real(dp) :: c(6, 1*ncb, 1*nck)
      real(dp) :: cb(6, 1*ncb)
      real(dp) :: t(15), tv(6), f(0:2), b(0:2)
      ...
      do kq = 1, nket
         ...
         do bq = 1, nbra
            ...
            tv(6) = t(15)
            do bc = 1, 1*ncb
               cb(1:6,bc) = cb(1:6,bc) + kab(bc,bq)*tv(1:6)
            end do
         end do
         if (.not. hit) cycle
         any = .true.
         do kc = 1, 1*nck
            do bc = 1, 1*ncb
               c(1:6,bc,kc) = c(1:6,bc,kc) + kcd(kc,kq)*cb(1:6,bc)
            end do
         end do
      end do
      res = 0.0_dp
   ...
   end subroutine hgp_dsss
end module hgp_dsss_m
