...
module cint_rotaxis_grad_2e
   ...
   use cint_rotaxis_kernels, only: ROTAXIS_LMAX, &
                                   ROTAXIS_KIND_S, ROTAXIS_KIND_P, ROTAXIS_KIND_L, ROTAXIS_KIND_D
   use cint_rotaxis_grad_kernels, only: rotaxis_grad_kernel, rotaxis_grad_has_class
   implicit none
   private

   public :: int2e_ip1_rotaxis_cart, int2e_ip1_rotaxis_sph, rotaxis_grad_supported
...
contains
   ...
   end function int2e_ip1_rotaxis_cart

   function int2e_ip1_rotaxis_sph(out, dims, shls, atm, natm, bas, nbas, env, ws) result(has_value)
      real(dp), intent(inout) :: out(0:)
      integer,  intent(in)    :: dims(0:), shls(0:), natm, nbas
      integer,  intent(in)    :: atm(0:), bas(0:)
      real(dp), intent(in)    :: env(0:)
   ...
   end function int2e_ip1_rotaxis_sph
   ...
   function rotaxis_drv(out, dims, shls, atm, natm, bas, nbas, env, sph) result(has_value)
      ...
      type(shell_t) :: sh(0:3)
      integer  :: x, ia, pr, ncomp_out(0:3), dim(0:3), m
      integer  :: i, j, k, l
      real(dp) :: cutoff
      ...
      do x = 0, 3
         ...
         if (sh(x)%sp) then
            ...
            case default
               error stop "cint_rotaxis_grad_2e: angular momentum beyond the generated kernels"
            end select
            if (sph) then
      ...
      if (.not. has_value) then
         ...
         do l = 0, dim(3) - 1
            do k = 0, dim(2) - 1
               do j = 0, dim(1) - 1
                  do i = 0, dim(0) - 1
                     out(i + dims(0)*(j + dims(1)*(k + dims(2)*l)) &
                         + m*dims(0)*dims(1)*dims(2)*dims(3)) = 0.0_dp
                  end do
               end do
            end do
         end do
   ...
   subroutine one_block(out, dims, sh, ncomp_out, env, cutoff, sph, has_value)
      ...
      integer  :: idr, odrv, bdrv
      real(dp) :: fb, fcc, fd

      ! 1. canonical order: ranks ascend within each pair, then the pairs.
      ! IDENTITY, not the energy driver's canonical order.  That driver may
      ! permute freely because the integral is symmetric under those swaps;
      ! d/dA names one centre, so the caller's slot 0 has to stay slot 0.
      ! The cost is that every ordered combination of kinds needs its own
      ! kernel rather than one per canonical class.
      perm = [0, 1, 2, 3]
      rk = sh%rank
      code = 0
      do m = 0, 3
      ...
      if (.not. rotaxis_grad_has_class(code)) then
         error stop "cint_rotaxis_grad_2e: quartet beyond the generated classes"
      end if
      ...
      ! three derivative directions, slowest, as int2e_ip1 has them
      ncomp = 3*nf(0)*nf(1)*nf(2)*nf(3)
      ncol  = ncb*nck
   ...
   contains
      ...
      subroutine body(wb, npb, wk, npk, nblk)
         ...
         real(dp) :: bp(6, npb), kab(wb, npb), kp(7, npk), kcd(wk, npk)
         ...
         call rotaxis_grad_kernel(code, nbra, ncb, bp, kab, nket, nck, kp, kcd, gc, cutoff, blk, has_value)
         ...
         ! THE DERIVATIVE INDEX.  It is a vector in the local frame like any
         ! other, so it rotates back with the same matrix -- and with nothing
         ! extra from the frame itself, because a gradient target is a
         ! combination of ordinary integrals, each frame-independent in
         ! value.  nb is now the product of the four nout, which is the
         ! stride at which the derivative direction changes.
         tmat = 0.0_dp
         tmat(1:3, 1:3) = rot
         call transform_index(blk, nb, 3, 3, ncol, tmat, work)
         ...
         ! int2e_ip1 writes the three derivative blocks one after another,
         ! each the size of the whole quartet block.
         odrv = dims(0)*dims(1)*dims(2)*dims(3)
         bdrv = nout(0)*nout(1)*nout(2)*nout(3)
                     ...
                     do ccb = 0, nctr(1) - 1
                        ...
                        do ib = 0, nout(1) - 1
                           ...
                           do cca = 0, nctr(0) - 1
                              ...
                              oa = ob + nout(0)*cca*sa
                              do ia = 0, nout(0) - 1
                                 out(oa + ia*sa + idr*odrv) = &
                                    blk(t0 + ia + idr*bdrv + 1)*fb*fc(0, ia)
                              end do
                           end do
                        end do
                     end do
   ...
   subroutine bra_pairs(sa, sb, env, rab, cutoff, bp, kab, nbra)
      ...
      real(dp), intent(out) :: bp(6, *), kab(sa%ntype*sb%ntype*sa%nctr*sb%nctr, *)
   ...
   subroutine index_matrix(l, is_sp, rot, sph, tmat, nout)
      ...
      case default
         error stop "cint_rotaxis_grad_2e: index_matrix beyond d"
      end select
...
end module cint_rotaxis_grad_2e
