...
module cint_hgp_grad_2e
   ...
   use cint_hgp_grad_kernels, only: hgp_grad_kernel, hgp_grad_has_class
   implicit none
   private

   public :: int2e_ip1_hgp_cart, int2e_ip1_hgp_sph, hgp_grad_supported
...
contains
   ...
   ! fallback.
   pure logical function hgp_grad_supported(shls, bas) result(yes)
      ...
      do x = 0, 3
         ...
         if (cint_bas_is_sp(shls(x), bas)) then
            code = 10*code + HGP_KIND_L
         else
            select case (l)
            case (0); code = 10*code + HGP_KIND_S
            case (1); code = 10*code + HGP_KIND_P
            case (2); code = 10*code + HGP_KIND_D
            case default
               yes = .false.
               return
            end select
      ...
      yes = hgp_grad_has_class(code)
   end function hgp_grad_supported

   function int2e_ip1_hgp_cart(out, dims, shls, atm, natm, bas, nbas, env, ws) result(has_value)
   ...
   end function int2e_ip1_hgp_cart

   function int2e_ip1_hgp_sph(out, dims, shls, atm, natm, bas, nbas, env, ws) result(has_value)
      real(dp), intent(inout) :: out(0:)
      integer,  intent(in)    :: dims(0:), shls(0:), natm, nbas
      integer,  intent(in)    :: atm(0:), bas(0:)
      real(dp), intent(in)    :: env(0:)
   ...
   end function int2e_ip1_hgp_sph
   ...
   function hgp_drv(out, dims, shls, atm, natm, bas, nbas, env, sph) result(has_value)
      ...
      type(shell_t) :: sh(0:3)
      integer  :: x, ia, pr, ncomp_out(0:3), dim(0:3), i, j, k, l, m
      real(dp) :: cutoff
      ...
      do x = 0, 3
         ...
         if (sh(x)%sp) then
            ...
            case default
               error stop "cint_hgp_grad_2e: angular momentum beyond the generated kernels"
            end select
            if (sph) then
   ...
   subroutine one_block(out, dims, sh, ncomp_out, env, cutoff, sph, has_value)
      ...
      ! IDENTITY, unlike the energy driver: d/dA names one centre, so the
      ! caller's slot 0 stays slot 0 and every ordered combination of kinds
      ! has its own kernel.
      perm = [0, 1, 2, 3]
      rk = sh%rank
      code = 0
      ...
      if (.not. hgp_grad_has_class(code)) then
         error stop "cint_hgp_grad_2e: quartet beyond the generated classes"
      end if
   ...
   contains
      ...
      subroutine body(wb, npb, wk, npk, nblk)
         ...
         real(dp) :: bp(11, npb), kab(wb, npb), kp(11, npk), kcd(wk, npk)
         ...
         call hgp_grad_kernel(code, nbra, ncb, bp, kab, nket, nck, kp, kcd, &
   ...
   subroutine bra_pairs(sa, sb, env, cutoff, bp, kab, nbra)
      ...
      real(dp), intent(in)  :: env(0:), cutoff
      real(dp), intent(out) :: bp(11, *), kab(sa%ntype*sb%ntype*sa%nctr*sb%nctr, *)
